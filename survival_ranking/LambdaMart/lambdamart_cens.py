...
def compute_lambda(true_scores, predicted_scores, good_ij_pairs, idcg):
	"""
	...
	# true_scores, predicted_scores, good_ij_pairs, idcg = args
...
def get_pairs(scores, censoring):
	"""
	...
	for i in range(len(scores)):
		for j in range(len(scores)):
			if (scores[i] < scores[j]) and (censoring[i] > 0):
				pairs.append((i,j))
	# pairs.append(pairs)
...
class LambdaMARTC:
	...
	def fit(self):
		...
		predicted_scores = np.asarray(random.choices(np.arange(1, 100), k=len(self.training_data))).astype(float) # np.zeros(len(self.training_data))
		predicted_scores = predicted_scores/max(predicted_scores)

		true_scores = self.training_data[:, 0]
		true_events = self.training_data[:, 1]
  
		tree_data = self.training_data[:,2:]

		good_ij_pairs = get_pairs(true_scores, true_events)

		# ideal dcg calculation
		idcg = ideal_dcg(true_scores) # [ideal_dcg(scores) for scores in true_scores]
		...
		for k in range(self.number_of_trees):
			...
			lambdas, w  = compute_lambda(true_scores, predicted_scores, good_ij_pairs, idcg)

			if self.tree_type == 'sklearn':
				# Sklearn implementation of the tree			
				tree = DecisionTreeRegressor()
				tree.fit(tree_data, lambdas)
				self.trees.append(tree)
				prediction = tree.predict(tree_data)
